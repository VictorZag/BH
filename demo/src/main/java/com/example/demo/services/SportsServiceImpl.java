...
public class SportsServiceImpl implements ItemService{

    private final ItemRepository itemRepository;

    @Override
    public void addItem(Item item) throws InvalidEntityException {
        if(item.getItemType() == ItemType.SPORTS)
        {
    ...
    /*    List<Item> items = new ArrayList<>();
        ...
        return items;*/
        return itemRepository.findAll(Example.of(Item.builder().itemType(ItemType.SPORTS).build()));
    }

    @Override
    public Long getItemsCount() {
        return itemRepository.count(Example.of(Item.builder().itemType(ItemType.SPORTS).build()));
